...
- **derpConcatenate UI**: Collapse/expand button on each signal entry toggles preview visibility (same as right-click), with "add"/"subtract" icon reflecting state. Linebreak separator between header and content, hidden when collapsed. Signal source names use `t_text_accent` color-key labels. Empty incoming signals display "Incoming signal is an {{t_text_error::empty string...}}" message.
