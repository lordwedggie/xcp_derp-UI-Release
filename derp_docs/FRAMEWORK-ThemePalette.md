...
| `node._derpStringPalette` | Per-node | The palette context used by multi color-key strings, defaulting to `{ path: "_system/_defaultPalette.json" }` or a category-specific default for `Dark`, `Light`, or `Neutral` themes. Future nodes can override this with their own palette file. |
