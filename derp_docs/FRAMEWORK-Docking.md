...
The outer-edge stack height resize path (`applyVerticalStackHeightResize` in `dockResize.js`) mirrors the horizontal stack width resize for vertical stacks. Dragging the top of the topmost node or the bottom of the bottommost node distributes the snapped pointer delta across all resizable (manual-height) members via `reconcileManualHeightsToTarget`, with each member clamped to its own `getVerticalResizeTargetMinHeight` (using `preserveExpandedFloor: true`). autoHeight members and collapsed members keep their original height (they are "fixed" during height distribution). The resize only stops when every resizable member has reached its min-height. Fixed members are identified by `canResizeVerticalMemberHeight` (returns `false` for collapsed or autoHeight nodes). The path is gated on `canResizeVerticalStackHeight` (entity must be at the top or bottom outer boundary of a non-Deck-Pressure vertical stack with at least one resizable member). Collapsed boundary growth is still handled by `applyCollapsedVerticalBoundaryResize` first; internal vertical seams are still handled by `applyVerticalStackSharedEdgeResize`. The new path only catches non-collapsed outer boundaries that fall through both. Heights are pinned during the live drag via `markDockResizeActiveMembers` with `preserveHeight: true`, which sets `_dockResizePreserveHeight` and `_isDerpResizing` on all stack members so the draw cycle does not override the manually distributed heights.
