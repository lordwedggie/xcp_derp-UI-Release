...
Horizontal stack corner affordance follows the same side-specific rule. A corner on an internal shared side only shows/uses width resize when `canResizeHorizontalSharedEdgeWidth()` passes for that side; a corner on an outer stack boundary uses `canResizeHorizontalStackWidth()`. Horizontal-stack corners are width-only affordances, so their DOM shield cursors should be `ew-resize` when active and hidden/default when blocked, never diagonal just because the grabbed node itself is manual-width.
...
- **`manual`**: Force-convert all stack members to manual on deck attach via `lockDeckStackMembersForAttach`. This extends `lockDeckNodeAxes` beyond the two attach-point nodes to all stack members, and forces the seam axis to manual at the preferred-value level. For vertical stacks, `_derpPreferredAutoHeight = false` and `autoHeight = false` are set on all members. For horizontal stacks, `_derpPreferredAutoWidth = false` is set on all members so seam eligibility opens; on left/right side Deck Pressure rows, runtime `autoWidth` is restored from `deckSavedAutoWidth` so Fatha layout sizing remains content-driven unless `_deckPressureSideHorizontalWidth` is explicitly set. The saved `deckSavedAutoHeight`/`deckSavedAutoWidth` (written by `saveDeckNodeAxes`) are preserved so restore-on-undock reverts to the original state.
...
3. **`lockDeckStackMembersForAttach`** snapshots `deckSavedAutoWidth` for every non-hub stack member before Manual-mode mutation, then restores runtime `autoWidth` from that saved value for left/right side rows in both Manual and Scale mode. For side="top"/"bottom", `lockDeckNodeAxes` forces `autoWidth = false` for vertical stack width sharing, and restoring it would break that.
