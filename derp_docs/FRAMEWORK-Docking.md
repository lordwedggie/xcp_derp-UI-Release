...
`handleNodeResize` in `fathaNodeResize.js` and the DOM shield's own-handle affordance (`canW`/`canH` in `fathaDOMshield.js`) both use `resolveDerpPreferredAutoHeight` and `resolveDerpPreferredAutoWidth` (the user's chosen Height/Width Mode) for resize axis eligibility, not runtime values. This is critical because `lockDeckNodeAxes` forces runtime `autoHeight = false` and `autoWidth = false` on docked/decked nodes, which would otherwise make `resolveDockResizeAxes` return `allowHeight/allowWidth: true` for preferred-auto nodes. By using preferred values, corner handles and pure top/bottom handles on vertical stacks correctly block height resize for autoHeight nodes even when the node is docked to a deck, and horizontal stack seams are blocked for autoWidth nodes. The shield does not render resize handles or cursors that the resize path would reject. All-auto stacks therefore have seam resize completely disabled (no manual members to absorb the delta), while mixed stacks allow resize that distributes only to manual members.

`resolveDerpPreferredAutoWidth` mirrors `resolveDerpPreferredAutoHeight`: it checks `_derpPreferredAutoWidth` first (set by Manual mode), then `deckSavedAutoWidth` (the original saved value), then falls back to runtime `autoWidth`. Manual mode sets `_derpPreferredAutoWidth = false` to open seams; Scale mode restores it from `deckSavedAutoWidth`.
...
- **`manual`**: Force-convert all stack members to manual on deck attach via `lockDeckStackMembersForAttach`. This extends `lockDeckNodeAxes` beyond the two attach-point nodes to all stack members, AND forces the seam axis to manual at both runtime and preferred levels. For vertical stacks, `_derpPreferredAutoHeight = false` and `autoHeight = false` are set on all members. For horizontal stacks, `_derpPreferredAutoWidth = false` and `autoWidth = false` are set on all members. This is necessary because seam eligibility checks (`canResizeVerticalMemberHeight`, `canResizeHorizontalMemberWidth`) use the **preferred** value (`resolveDerpPreferredAutoHeight` / `resolveDerpPreferredAutoWidth`), and `lockDeckNodeAxes` only forces runtime values to `false` while restoring the preferred value. The saved `deckSavedAutoHeight`/`deckSavedAutoWidth` (written by `saveDeckNodeAxes`) are preserved so restore-on-undock reverts to the original state.
