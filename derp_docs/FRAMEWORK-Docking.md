...
- Active-resize and fresh manual seam-fit height preservation in left/right vertical Deck Pressure branches must keep live member heights during non-height drag paths when fitting would compress the branch, even if the actively dragged pair is temporarily below measured floors or the current branch total temporarily exceeds the side band. When an active Deck frame resize is changing height, the branch should fit the live side-band target both upward and downward from current live heights instead of recomputing from minimums. Otherwise pressure layout may redistribute height into unrelated siblings during the drag. Non-active pressure layout may still refit/clamp inside the fixed frame.
