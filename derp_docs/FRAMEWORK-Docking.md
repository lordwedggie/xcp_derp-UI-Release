...
### <span style="color: #80ffc0">Resize eligibility uses preferred autoHeight, not runtime</span>
`handleNodeResize` in `fathaNodeResize.js` and the DOM shield's own-handle affordance (`canH` in `fathaDOMshield.js`) both use `resolveDerpPreferredAutoHeight` (the user's chosen Height Mode) for resize axis eligibility, not `resolveDerpRuntimeAutoHeight`. This is critical because `lockDeckNodeAxes` forces runtime `autoHeight = false` on docked/decked nodes, which would otherwise make `resolveDockResizeAxes` return `allowHeight: true` for preferred-autoHeight nodes. By using preferred autoHeight, corner handles and pure top/bottom handles on vertical stacks correctly block height resize for autoHeight nodes even when the node is docked to a deck, and the shield does not render height-resize handles or cursors that the resize path would reject. All-autoHeight stacks therefore have height resize completely disabled (no manual members to absorb the delta), while mixed stacks allow height resize that distributes only to manual members. autoWidth remains single-layer (runtime only) — there is no preferred-autoWidth concept.
...
- **`manual`**: Force-convert all stack members to manual on deck attach via `lockDeckStackMembersForAttach`. This extends `lockDeckNodeAxes` beyond the two attach-point nodes to all stack members, AND forces the seam axis to manual at both runtime and preferred levels. For vertical stacks, `_derpPreferredAutoHeight = false` and `autoHeight = false` are set on all members — this is necessary because `canResizeVerticalMemberHeight` checks the **preferred** value (`resolveDerpPreferredAutoHeight`), and `lockDeckNodeAxes` only forces runtime `autoHeight = false` while restoring the preferred value. For horizontal stacks, `autoWidth = false` is set on all members — this opens seam eligibility because `canResizeHorizontalMemberWidth` checks **runtime** `autoWidth` (autoWidth is single-layer; there is no preferred-autoWidth concept). The saved `deckSavedAutoHeight`/`deckSavedAutoWidth` (written by `saveDeckNodeAxes`) are preserved so restore-on-undock reverts to the original state.
...
3. **`lockDeckStackMembersForAttach`** in Scale mode also restores `autoWidth` from `deckSavedAutoWidth` for all non-hub members, but ONLY when `side` is "left"/"right" (not "top"/"bottom"). For side="top"/"bottom", `lockDeckNodeAxes` forces `autoWidth = false` for vertical stack width sharing, and restoring it would break that. For side="left"/"right", `lockDeckNodeAxes` does NOT touch `autoWidth`, so Manual mode corruption persists unless explicitly restored here.
