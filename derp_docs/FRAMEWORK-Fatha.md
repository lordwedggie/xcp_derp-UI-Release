...
- **Seed V3 Idle Draw Cache:** `derpSeedV3` can use a Fatha-level bitmap draw cache while idle/static, including when docked. The cache is disabled during press, editor focus, resize, drag, or awake animation frames, and its key includes layout hash, seed history, mode, title, busy/bypass state, selected state, hovered region, size, scale bucket, and theme session. Selected and hovered Seed V3 nodes cache that visual state instead of animating every frame. If Deck Pressure leaves `_forceSync`/`_layoutDirty` set but that visual key still matches an existing cache, Seed V3 can reuse the bitmap and clear those stale flags instead of paying a full redraw.
- **Content Viewport (experimental):** `scrollViewport: true` layout regions can cap visible content to `clipHeight`, draw a right-side scrollbar only while overflowing, and keep full internal content scrollable. The framework code lives in `core/fathaContentViewport*.js`; `FATHA_CONTENT_SCROLLBAR_WIDTH` is the current reserved scrollbar width knob, and `window.xcpDerpDebugContentViewports = true` draws viewport debug boxes.
  ...
  The system panel exposes one shared `Height mode` FILEBROWSER. Standard nodes use `Auto`/`Manual` as the user-facing wrapper for `autoHeight`. Clipped nodes can override the same control through `getDerpHeightModeConfig()`: `derpLoraStack` and `derpTriggerWall` show `Fit Node` plus `1`-`5` measured entry/group options. `Fit Node` keeps the outer node manual-height and lets the viewport fit the current node/stack/deck height; numeric modes make the node derive its height floor from that many measured rows/groups before overflow scrolls. Viewport overflow must not force manual-height nodes back into auto-height sizing.
