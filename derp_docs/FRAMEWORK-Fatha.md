...
- **Content Viewport (experimental):** `scrollViewport: true` layout regions can cap visible content to `clipHeight`, draw a right-side scrollbar gutter only while overflowing, and keep full internal content scrollable. The framework code lives in `core/fathaContentViewport*.js`; `FATHA_CONTENT_SCROLLBAR_GUTTER` is the current 10px width knob, and `window.xcpDerpDebugContentViewports = true` draws viewport debug boxes.
