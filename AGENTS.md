...
- The engine overwrites a region's `key` config property with the map slot name (`currentRegion = { ...localCfg, key: <slotName>, ... }` in masterLayoutEngine.js). Any side-channel cache keyed by `config.key` (e.g. `widget_ImageHTML`'s `_imageInstanceCache`/`_videoInstanceCache`) is therefore keyed by the **region slot name**, never the `key:` property written in the layout map. External lookups (e.g. `getDerpVideoDeckVideoEl`) must use the slot name — a custom `key:` mismatch kills playback/seek/volume silently while posters and metadata callbacks keep working.
- Multi-viewport clip-delta handoff: When a node has two+ `scrollViewport` regions in the same vertical stack, `applyContentViewportLayout` runs all `clipHeight` resolvers against pre-shift `region.y` in one pass. The earlier viewport's resolver must store its clip delta on `node._<name>ClipDelta`; the later viewport's resolver adds it to `available` to correct for the post-clip shift that hasn't happened yet. `derpConcatenate` (`regionSignalsViewport` → `regionConcatContent`) is the canonical example. See `FRAMEWORK-Clipping.md`.
- Viewport `fullHeight` measures descendant `(y - region.y) + h` with NO margins. Content insets that must scroll with the content (sH/sW text margins) belong on the innermost content child so the engine folds them into an auto-height wrapper's measured `h`; margins on the wrapper/direct child are silently dropped and the clip slices the bottom content line. Wrappers carry no margins (derpConcatenate `regionConcatContentInner`). See `FRAMEWORK-Clipping.md`.
- Viewport `clipHeight` resolvers on manual-height nodes with a bottom-pinned sibling row (two-sibling fill + flow-auto pattern) must clamp `available` to `regions.<bottomRegion>.y`, not `nodeH - footer` — otherwise the clipped region overlaps the bottom row (loader `loaderRegion` is canonical; derpConcatenate `addSignalRegion`). Floor the result at `minClipHeight` so the viewport never disables into full overflow. See `FRAMEWORK-Clipping.md`.
- A viewport `clipHeight` resolver whose bottom clamp is invalid (PASS 1 zero-height measurement, bottom row unpositioned) must report the declared min clip, never a physical `nodeH`-based measurement. PASS 1 feeds `layout.contentMinHeight`; a current-height clip inflates it to the node's present size and makes standalone manual resize expand-only (derpConcatenate dual viewports). See `FRAMEWORK-Clipping.md`.
- The compact resize-floor walk (`getVisibleRegionLayoutFloor` in dockResize.js) must not use the live measured `region.h` of auto-height regions whose subtree contains a content viewport — live heights track the current clip or full unclipped content and pin the floor to the node's present height, making standalone manual resize expand-only. Viewport-bearing subtrees floor at config minima + `childTotal` (which carries the viewport's `_contentViewportMinClipHeight`). See `FRAMEWORK-Clipping.md`.
- Hashes prevent rebuilds. Every parameter that changes visible output must be represented in the appropriate hash.
- `ignoreLayout: true` regions (drag-preview floaters with absolute config `x`/`y`) must not advance the column/row flow cursor in `processRecursive` — their config `x`/`y` survive via the non-reserved keyword copy and override flow position, so letting the cursor jump past them pushes every later sibling to the drag position. In auto-height containers that feeds `contentMinHeight`, and bottom-anchored auto-height Bastas grow without bound while dragging to the tail (bastaPagesBook was the canonical victim; loader/concatenate floaters were protected only by manual node heights or last-child luck).
- **Basta singleton resurrection NEVER re-resolves the theme** — the resurrection path in `spawnBasta` merges `config.properties` but never re-reads `properties.selectedTheme` or re-runs `handleThemeUpdate`, so a panel first spawned under a different theme keeps rendering it forever. The BastaInstance constructor is the ONLY place the theme resolves (`this.themeName = config.themeName || host.properties.selectedTheme`). For a basta that must follow the host's per-node theme, use the bastaLoraDetail pattern: `existing.destroy()` + fresh `spawnBasta` on every open (NOT singleton resurrection) — the fresh constructor re-hydrates the host's CURRENT theme, and saved `bastaOffset_`/`bastaSize_` (persisted on host properties) are restored by the constructor so user position/size survive. bastaPagesBook was the canonical victim (spawned under default, then host rethemed — panel stayed default until the destroy+respawn fix). Do NOT try to fix this by re-running `onThemeUpdate` on the resurrected instance or by changing the paint cascade — resurrection leaves stale render state (mid-fade alpha, cached base map, comp-data, DOM element styles) that re-hydration can't reliably flush.
- `drawHeader` toggles `headerRegion.hidden` and `footerGapHeight` in the shared base layout map. Custom-hash nodes that do not include `drawHeader` in their `_layoutMapHash` will serve stale layouts after a header toggle. Either include `drawHeader` in the custom hash, or rely on the generic `_headerInsetLayoutHash` which already includes it.
- Engine-side cache keys must combine the node structure hash with the header inset hash via `getDerpLayoutCacheHash(node)` (dockDimensions.js) — never read `_layoutMapHash` raw in layout engine, Deck Pressure, resize session, Basta, or whole-wall cache keys. `_headerInsetLayoutHash` is written by `getVirtualNodeLayoutMap`; writing it to `_layoutMapHash` clobbers the node's own structure hash and kills map caching.
...
- PASS 1 wrap-text inflation: `wrap: true, width: "full"` widgets measured during PASS 1 must NOT use `context.geometry.w` (= `SQUISH_WIDTH = 10`) as the wrap boundary — it wraps normal strings into dozens of lines and inflates `contentMinHeight` 5–10×, which `settleDerpSizeBeforeDrawImpl` then consumes as the auto-height (derpTutorial was the canonical victim: ~922px instead of ~118px). `masterLayoutEngine.runLayoutPass` propagates `isMeasurePass: !!this._measurePass` through the context (including `footerRegion`); `interpretLayoutProps` in `widgetsUtils.js` checks `context.isMeasurePass === true` and uses `context.owner.size[0] - padW - 20` as the wrap boundary instead. Any future code that derives a wrap width from `context.geometry.w` must respect `isMeasurePass` the same way.
...
- Content viewport scrollbar gutter is internal viewport chrome, not node content width. Keep `layout.intrinsicContentMinWidth`, `layout.contentMinWidth`, `layout.totalWidth`, and `panelBackground.w` unchanged when overflow appears. Use existing right-side node margin exactly as the scrollbar lane when available; do not widen it to the fallback gutter. Otherwise narrow the viewport rect/descendant lane just enough to fit the scrollbar. Keep the painted track centered in that lane so the left and right gaps match.
...
- "Visible-count-follows-space" lists (derpSeedV3 Auto Logs is canonical): pad the display rows to a fixed cap with inert placeholders and let the viewport `clipHeight` resolver floor the fit-available height to whole rows. The visible count then tracks live resize (standalone/stacked/decked) with ZERO structural map rebuilds and costs nothing when no layout pass runs. Never rebuild the map per resize frame to add/remove rows, and keep data retention (e.g. `seedHistoryLimit`) decoupled from the display row count so shrinking never deletes real entries.
...
- `getLinearResizeMembers` lives canonically in `dockResizeSharedEdges.js` (exported); dockResize, fathaHandler, and fathaDOMshield import it from there. Do not re-add local copies — a stale copy missing the pressure-hub early-return guard already caused shield/resize divergence once.
- Delayed lock-release timers must be session-aware. `_horizontalDeckWidthResizeLock` releases go through `scheduleHorizontalDeckWidthLockRelease()` (monotonic stamp + newest-session node set); a bare `setTimeout` clearing a boolean lock lets a stale timer kill a newer session's lock. Keep such locks boolean — readers check `=== true`.
- Keep resize affordances and resize execution in sync. DOM shield hitboxes, cursors, seam ghosts, and JS resize handlers should share the same eligibility helpers so handles do not appear for rejected resize paths. Seam ghost drawing re-checks `canResizeVerticalSeamPair` / `canResizeHorizontalSeamPair` at the draw site (`getSeamGhostPairsForSession` in `masterDockEngine.js`), not just at session-creation time — hover sessions can survive past pointer-leave or eligibility changes (neighbor toggled to autoHeight/collapsed) and would otherwise paint ghosts on seams that can no longer be dragged. Deck Pressure hub/side-width seam ghosts are gated at session creation and are not re-checked at the draw site.
- Deck frame top/bottom outer-edge and hub-seam hitboxes extend outward by `SHIELD_CORNER_RESIZE_OUTWARD_PAD` (same pad as the corner handles) so the grab zone straddles the visual frame edge — the body drop shadow's `shadowOffsetY` pushes the perceived bottom edge below `node.size`, and an inward-only strip reads as "above the edge".
...
- Deck Pressure side-width seam resize must also preserve content viewport `clipHeight` for clipped branch members through live resize and short post-release settlement. Width-only side seams must not let loader/Lora/Seed viewports recompute taller visible windows from transient pressure/layout measurements. When clipped branch members are present, exact side-height snapshots must still fill the ImageDeck side band by assigning spare height to non-clipped expanded members when available; plain non-viewport branches still preserve raw member heights during width-only side resize.
- Deck Pressure viewport-backed side-branch detection must inspect both live `_contentViewportState`/regions and the declarative `layoutMap`. Initial attach/pressure passes can run before live viewport state exists; relying only on live state lets clipped loaders become accidental height fillers or leaves all-clipped branches short.
- Deck Pressure pressure-layout and min-span caches must include viewport signatures plus layout height minima. Otherwise enabling clipping or changing loader content can leave side branches short until an unrelated deck resize wakes pressure layout.
- Internal vertical seams in left/right Deck Pressure side branches must use branch-only members, use compact viewport floors, preserve the canonical Deck frame, and immediately rerun Deck Pressure layout. Capture that frame from the current physical Deck bounds, not a freshly recomputed pressure plan, and preserve the branch's collapse state while the seam is live. Do not preserve viewport `clipHeight` on height seams; the clipped viewport must recompute to the new physical height.
...
- Deck Pressure outer-frame edge drags resize the decked stacks, never the hub: left/right outer edges resize the side stack width via `_deckPressureSideWidthOverrides` + an expanded preserved frame; top/bottom outer edges resize the top/bottom stack height via `_deckPressureTopBottomHeightOverrides` + an expanded preserved frame. Both overrides are consumed in `computeDeckPressureGeometryPlan` so the hub rect stays fixed while `applyDeckPressureLayout` refits the stacks, and both are cleared with the resize session in `clearPressureSession`. A top/bottom edge with no covering top/bottom branch (`branchCoverage <= 0.5`) is the hub's own edge — keep the legacy hub-driven path there.
- Branch-member pass-through in `startResize` (fathaDOMshield.js) must stay in sync with the same seam predicate the ghost/cursor/hit-test paths use. A pass-through that swallows `left`/`right` on a Deck Pressure branch member re-invokes `shield.onpointerdown`, which re-detects the same seam and recurses — the drag never starts even though the ghost and cursor still appear. Gate the pass-through on `!canResizeHorizontalSharedEdge(...)` so real internal sibling seams (top/bottom horizontal branches, side-horizontal branches) still drag.
- The Fatha/Uncle draw loops must stand down while `hub._deckPressureFrameHeightResizeActive` is set (gate via `isDeckPressureFrameHeightResizeActive(node)` from fathaHandler.js): pin member width AND height targets to the plan-assigned physical size, and skip the horizontal shared-height sync, `balanceHorizontalDeckWidthChange`, and auto-axis `properties.nodeSize` writes — during a top/bottom outer-frame edge drag the Deck Pressure plan owns member geometry on both axes, and per-frame auto targets overwrite it, causing flicker/disappearing nodes/gaps/width oscillation mid-drag. See `FRAMEWORK-Docking.md`.
- Top/bottom frame edge drags must also freeze row member widths: snapshot them into `hub._deckPressureTopBottomWidthOverrides` at drag start (`applyDeckPressureFrameHeightResize`) and consume the snapshot in `computeDeckPressureGeometryPlan` via `getDeckPressureFrameDragRowWidths` instead of `fitDeckPressureRowWidths`. The live refit weights manual members by `liveWidth - minWidth`, so transient self-inflation (derpSeedV3's width-floor sync) feeds back into the plan and the member ratchets wider until mouse release. A height-only drag never rewrites widths. See `FRAMEWORK-Docking.md`.
...
- Left/right Deck Pressure vertical branch seams are height-resizable when at least one seam member is manual-height; a preferred-auto member's height refits from the side band while the manual partner absorbs the delta. A seam between TWO preferred-auto members is not resizable — the side band owns both heights, no member can absorb the delta — enforced in `canResizeVerticalSeamPair` so ghost/hitbox/cursor/drag all stay in sync. Keep ordinary auto-height vertical stacks blocked.
- Top/bottom hub seams (horizontal branch row ↔ ImageDeck hub) are height-resizable via `applyDeckPressureTopBottomHeightResize`, mirroring side-width seams: branch row height changes, hub height compensates inside the preserved frame, widths stay frozen via `_deckPressureTopBottomWidthOverrides`. Eligibility is gated at `resolveDeckPressureTopBottomSeam` (all-preferred-auto rows show no affordance at all); ghosts come from the same `getDeckPressureSideSeamGhost`/`getDeckPressureSideHoverGhost` helpers as side seams. See `FRAMEWORK-Docking.md`.
...
- Content viewport and clipped-node resize floors belong in `D:\_AI_KnowledgeBase\derp-UI\framework\FRAMEWORK-Clipping.md`; use `minClipHeight` / `_contentViewportMinClipHeight` for viewport-backed floors and preserve viewport scroll targets across transient measurement passes. Deck Pressure compact viewport floors (`getDeckPressureCompactViewportMinHeight`) must delegate to the `getVerticalResizeTargetMinHeight(..., { preserveExpandedFloor: false, ignoreViewportLayoutFloor: true })` layout walk — summing viewport states + header/footer misses fixed interleaved rows and under-floors multi-viewport nodes (derpConcatenate) and loaders.
...
- Swallowed-drag resize exits (handled with no geometry change, e.g. collapsed-boundary `targetIndex < 0` / `delta === 0` exits) must still set `result.liveResize = true`; `applyDockResizeResult()` keys shield batching off that flag. See `.agents/references/dock-live-resize-shield-sync.md`.
- `LAYOUT_RESERVED_KEYS` in `dockResize.js` must mirror `RESERVED_KEYWORDS` in `masterLayoutEngine.js` (including `scrollViewport`, `clipHeight`, `contentViewportClip`); a diverged list lets config keys be mistaken for child regions in compact floor walks.
- `applyDeckPressureLayout()` reuses its preliminary geometry plan as the final plan when the collapse passes changed nothing; every inter-plan mutation must route through `markChanged` or the reuse goes stale. See `FRAMEWORK-Docking.md`.
- `getDeckPressureMinSpanForState` runs re-entrant — branch members reach it mid-draw via `normalizeDerpDockedLayout` → `applyDeckPressureLayout`, after their own `layout.compute` but before paint. Its simulated collapsed/expanded measurement computes must therefore leave the live layout in the node's REAL collapse state at its REAL height, and must snapshot/restore `_contentViewportScroll` so the tiny probe height cannot clamp user scroll away. A restore pass that recomputes the expanded map at the collapsed probe height (`snap*2`) gets painted the same frame: anchored fills collapse, bottom-pinned loader rows jump to the node top, and stale measured minima can overgrow a side stack past the deck band. See `FRAMEWORK-Docking.md`.
...
- Internal vertical seam sessions must freeze `topMinH`/`bottomMinH` at session creation. Viewport-clipped nodes have `contentMinHeight` that changes with node height, creating a feedback loop if the min is re-read each pointermove. See `FRAMEWORK-Docking.md`.
- Same-frame deck caches keyed on `frame` alone must fold in the deck graph index stamp (`getDeckGraphIndexStamp(graph)`): dock/undock commits (`setPeerDeckNeighbor` → `invalidateDeckGraphIndex`) and graph swaps happen mid-frame, and a frame-only cache serves pre-commit deck membership for the rest of the frame (the same-frame third-node-attach regression). `getDeckFrameState`'s cache generation is `(deckCacheFrame, deckCacheIndexStamp)`. See `FRAMEWORK-Optimization.md`.
...
- **Node-id guards must be string-safe: this ComfyUI frontend assigns node ids as STRINGS** (`"-1"` before `graph.add`, `"80"` after). Strict `=== -1` / `!== -1` comparisons are type-defeated — creation-time theme updates (`onNodeCreated` → theme register → `onThemeUpdate` → `syncDerpOutputs`) then write ghost signals under the shared key `"-1:0"` that every fresh node overwrites, and the concat picker's self-filter can never match it (the node listed ITSELF under the ghost id). The fix pattern (derpConcatenate): `Number(node?.id) === -1` helper, ghost `"-1"` base-id filter in signal picker items, and a sync fingerprint that includes the signalId so the first post-add sync self-heals any parked ghost registration. The six loader cores (`js/derps/loaders/core/`) and the shared wireless picker (`bastaSignalReceiver.js`, negative base-id filter) are fixed; remaining `=== -1` sites (Seed, TriggerWall, LoraStack, SignalOut...) keep the same latent defeat — fix them opportunistically when touching those files, not in one sweeping change.
- **Scrub/drag paths must never rebuild layout maps per pointermove.** VideoDeck video frames are painted by `drawImage` on the MAIN thread (hidden `<video>` in `widget_ImageHTML`'s `_videoInstanceCache`, NOT a composited DOM element), so per-move `refreshNodeLayoutMap()` + `_layoutMapHash = null` churn starves frame presentation — the scrub "sticks then jumps". The smooth pattern (derpVideoDeck seek slider): update all cached layers in place (live `layout.regions`, `_compDataCache`, AND the `this.layoutMap` object the engine copies regions from), coalesce the media element's `currentTime` sets through a rAF chaser (`scheduleVideoDeckSeekApply` — latest target wins, retry while `seeking`), suppress landed-frame self-correction while `_pressedRegionKey` owns the drag, and do ONE clean rebuild in `onDragEnd` (`finalizeVideoDeckSeekDrag`).
...
- **Per-frame `setDirtyCanvas(true, false)` is a canvas-wide nuke.** LiteGraph treats it as "dirty the entire canvas next frame", forcing every node's `onDrawForeground` to run — not just the calling node. Calling it 60 times/sec from an animation path (e.g. the old selection pulse) makes every non-selected node go red in the performance tracker and cuts frame rate by ~50%. When an animation is quantized to a slower bucket (the selection pulse uses `PULSE_FRAME_BUCKET_MS = 50` → ~20fps visual change), gate the dirty call through a `requestAnimationFrame` waker that only fires when the bucket actually changes. Never call `setDirtyCanvas` from a per-frame draw path unless the visual actually changed this frame. The selection pulse waker lives in `fathaHandler.js` (`_pulseWaker` / `_registerPulseNode` / `_tickPulseDirty`) and the user toggle is `Derp.SelectionPulseAnimation` (registered in `masterSettings.js`); when the toggle is off, selected nodes render a static `_ON` frame instead of falling through to the `paintOFF` default.
