...
- Internal vertical seams in left/right Deck Pressure side branches must use branch-only members, use compact viewport floors, preserve the canonical Deck frame, and immediately rerun Deck Pressure layout. Do not preserve viewport `clipHeight` on height seams; the clipped viewport must recompute to the new physical height.
