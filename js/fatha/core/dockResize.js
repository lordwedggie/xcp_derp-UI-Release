...
import {
    ...
    getDockNodeMinWidth,
    getActiveVerticalNodeWidthLock,
    getSharedDockHeight,
...
export function settleDerpSizeBeforeDrawImpl(entity, options = {}, deps = {}) {
    ...
    const activeVerticalWidthLock = isHeightOnlyResize ? getActiveVerticalNodeWidthLock(entity, 0) : 0;
    const targetW = activeVerticalWidthLock > 0
        ? activeVerticalWidthLock
        : isHeightOnlyResize && currentFrameworkW > 0
            ? currentFrameworkW
...
export function resolveHorizontalDeckSharedHeightImpl(node, deps = {}) {
    ...
    return members.reduce((maxHeight, member) => {
        const memberVars = typeof member?.getDerpVars === "function"
            ...
            : getDerpVars(member);
        const snap = Number(memberVars?.SNAP) || 10;
        if (canResizeHorizontalMemberHeight(member, graph)) {
            const manualHeight = Math.max(
                Number(member?.size?.[1]) || 0,
                Number(member?.properties?.nodeSize?.[1]) || 0,
                getVerticalResizeTargetMinHeight(member, snap)
            );
            return Math.max(maxHeight, manualHeight);
        }
        const measured = {
...
export function handleDerpComputeSizeImpl(entity, out, minWidth = 100) {
    ...
    const activeVerticalWidthLock = entity?._dockResizePreserveHeight === true ? getActiveVerticalNodeWidthLock(entity, 0) : 0;
    const minW = (entity?._isDerpResizing === true && entity?._dockResizePreserveHeight === true)
        ? Math.min(layoutMinW, activeVerticalWidthLock || Number(entity?.size?.[0]) || layoutMinW)
        : layoutMinW;
...
function applyCollapsedVerticalBoundaryResize(entity, resizeAnchor, requestedEntityHeight, snap, result, addCounterpart, graph) {
    ...
    if (isTopBoundary) {
        members.forEach((member, index) => {
            ...
            syncDeckNodeSize(member, getDockNodeWidth(member), height, { silent: true, deferDirty: true, deferSync: true, liveResize: true });
...
function reconcileManualHeightsToTarget(nextHeights, manualMembers, originalHeights, targetManualTotal, minH, snap) {
    const unit = Math.max(1, Number(snap) || 10);
    const mins = new Map(manualMembers.map((member) => [member.id, getVerticalResizeTargetMinHeight(member, snap, { preserveExpandedFloor: true })]));
    const minTotal = manualMembers.reduce((sum, member) => sum + (mins.get(member.id) || 0), 0);
    const targetTotal = Math.max(minTotal, Number(targetManualTotal) || 0);

    manualMembers.forEach((member) => {
        const minHeight = mins.get(member.id) || 0;
        nextHeights.set(member.id, Math.max(minHeight, originalHeights.get(member.id) || 0));
    });

    let currentTotal = manualMembers.reduce((sum, member) => sum + (nextHeights.get(member.id) || 0), 0);
    let diff = targetTotal - currentTotal;
    let growIndex = 0;
    ...
    while (Math.abs(diff) >= unit - 0.5) {
        const order = diff > 0
            ...
            : manualMembers
                .slice()
                .sort((a, b) => ((nextHeights.get(b.id) || 0) - (mins.get(b.id) || 0)) - ((nextHeights.get(a.id) || 0) - (mins.get(a.id) || 0)));
        let adjusted = false;
        for (let i = 0; i < order.length; i += 1) {
            const member = diff > 0 ? order[(growIndex + i) % order.length] : order[i];
            const current = nextHeights.get(member.id) || 0;
            if (diff > 0) {
                nextHeights.set(member.id, current + unit);
                diff -= unit;
                growIndex += i + 1;
                adjusted = true;
            } else {
                const minHeight = mins.get(member.id) || 0;
                if (current - unit < minHeight - 0.5) continue;
                nextHeights.set(member.id, current - unit);
                diff += unit;
                adjusted = true;
            }
    ...
    return manualMembers.reduce((sum, member) => sum + (nextHeights.get(member.id) || 0), 0);
}
...
function applyVerticalStackHeightResize(entity, resizeAnchor, requestedEntityHeight, minH, snap, result, addCounterpart, graph) {
    ...
    markDockResizeActiveMembers(entity, members, entity, { preserveHeight: true });

    const currentSession = entity._dockResizeSession;
    const sessionSide = isTopHandle ? "stack-top" : "stack-bottom";
    const sessionMatches = currentSession
        && currentSession.side === sessionSide
        && currentSession.entityId === entity.id;
    if (!sessionMatches) {
    ...
    const requestedDelta = Number.isFinite(explicitDelta)
        ? explicitDelta
        : Number(requestedEntityHeight) - entityStartHeight;
    ...
    const anchorY = isTopHandle
        ? members.reduce((max, member) => {
            const startPos = session.stackStartPositions?.[member.id] || member.pos || [0, 0];
            return Math.max(max, (Number(startPos[1]) || 0) + (originalHeights.get(member.id) || 0));
        }, Number.NEGATIVE_INFINITY)
        : members.reduce((min, member) => {
            const startPos = session.stackStartPositions?.[member.id] || member.pos || [0, 0];
            return Math.min(min, Number(startPos[1]) || 0);
        }, Number.POSITIVE_INFINITY);

    const fixedHeight = members
        .filter((member) => !canResizeVerticalMemberHeight(member, graph))
        .reduce((sum, member) => sum + (originalHeights.get(member.id) || 0), 0);
    const manualMinTotal = manualMembers.reduce((sum, member) => sum + getVerticalResizeTargetMinHeight(member, snap, { preserveExpandedFloor: true }), 0);
    const originalManualTotal = manualMembers.reduce((sum, member) => sum + (originalHeights.get(member.id) || 0), 0);
    const targetManualTotal = Math.max(manualMinTotal, originalManualTotal + snappedRequestedDelta);
    const nextHeights = new Map(originalHeights);

    members.forEach((member) => {
        if (canResizeVerticalMemberHeight(member, graph)) return;
        nextHeights.set(member.id, originalHeights.get(member.id) || getDockNodeHeight(member));
    });

    const manualTotal = reconcileManualHeightsToTarget(nextHeights, manualMembers, originalHeights, targetManualTotal, minH, snap);
    const totalHeight = fixedHeight + manualTotal;
    ...
    dockDebug("resize-vertical-stack-height", () => ({
        ...
        resizeAnchor,
        requested: { requestedEntityHeight, requestedDelta, snappedRequestedDelta, requestedTotalHeight: totalHeight },
        fixedHeight,
        targetManualTotal,
        members: members.map(snapshotDockNode),
    }));
...
function applyHorizontalStackWidthResize(entity, resizeAnchor, requestedEntityWidth, minW, snap, result, addCounterpart, graph) {
    ...
    if (Math.abs(snappedRequestedDelta) < 0.5) {
        members.forEach((member) => {
            ...
            const posChanged = setDeckNodePos(member, Number(startPos[0]) || 0, Number(member.pos?.[1]) || 0);
            if ((sizeChanged || posChanged) && typeof member.syncUncleSlots === "function") member.syncUncleSlots();
            addCounterpart(member);
        });
...
function applyDeckPressureSideWidthResize(entity, resizeAnchor, requestedEntityWidth, minW, snap, result, addCounterpart, graph) {
    ...
    if (branchAxis === "horizontal") {
        ...
        branchMembers.forEach((member) => {
            ...
            member._horizontalDeckWidthBalanceReady = true;
            syncDeckNodeSize(member, width, getDockNodeHeight(member), { silent: true, deferDirty: true, deferSync: true, liveResize: true });
            if (typeof member.syncUncleSlots === "function") member.syncUncleSlots();
            addCounterpart(member);
        });
...
export function syncDockResizePair(entity, resizeAnchor, newW, newH, minW, minH, snap = 10) {
    ...
    if (horizontalResizeMembers.length > 1 && requestsHeightResize && canResizeHorizontalStackHeight(entity, graph)) {
        markDockResizeActiveMembers(entity, horizontalResizeMembers, entity, { preserveHeight: true });
        ...
        const groupMinH = horizontalResizeMembers.reduce((maxMin, node) => {
            return Math.max(maxMin, getVerticalResizeTargetMinHeight(node, snap));
        }, snap * 4);
        const snappedHeight = Math.max(snapResizeValue(newH, snap), groupMinH);
        const topY = horizontalResizeMembers.reduce((minY, node) => Math.min(minY, Number(node?.pos?.[1]) || 0), Number.POSITIVE_INFINITY);
        const startRowHeight = getSharedDockHeight(horizontalResizeMembers, getDockNodeHeight(entity));
        const nextY = (isTopHandle && Number.isFinite(topY)) ? topY + (startRowHeight - snappedHeight) : topY;
        horizontalResizeMembers.forEach((node) => {
            const nodeW = getDockNodeWidth(node);
            syncDeckNodeSize(node, nodeW, snappedHeight, { silent: true, deferDirty: true, deferSync: true, liveResize: true });
            if (Number.isFinite(nextY)) setDeckNodePos(node, Number(node.pos?.[0]) || 0, nextY);
    ...
    if (result.handledHeight && horizontalResizeMembers.length > 1) return result;
    ...
    if (requestsHeightResize && applyVerticalStackHeightResize(entity, resizeAnchor, newH, minH, snap, result, addCounterpart, graph)) {
        return result;
    }
