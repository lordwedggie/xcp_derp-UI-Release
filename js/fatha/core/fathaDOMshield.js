...
function isDeckPressureFrameEdge(node, graph, side) {
    ...
    const h = Number(node?.size?.[1] ?? node?.properties?.nodeSize?.[1]) || 0;
    const tolerance = 1;
    if (side === "left") return Math.abs(x - bounds.left) <= tolerance;
    if (side === "right") return Math.abs((x + w) - bounds.right) <= tolerance;
    if (side === "top") return Math.abs(y - bounds.top) <= tolerance;
    if (side === "bottom") return Math.abs((y + h) - bounds.bottom) <= tolerance;
    return false;
}
...
// Top/bottom hub seam: the edge between a horizontal branch row (decked above
// or below the hub) and the hub itself. Eligibility-gated at the resolver so
// hitbox, cursor, hover ghost, and drag start all stay in sync — a row whose
// members are all preferred-auto-height cannot absorb the delta and shows no
// seam affordance at all.
function resolveDeckPressureTopBottomSeam(node, graph, side) {
    if (side !== "top" && side !== "bottom") return null;
    const pressureHub = graph ? getDeckPressureHubForNode(node, graph) : null;
    if (!pressureHub) return null;
    if (pressureHub.id === node?.id) {
        return canResizeDeckPressureTopBottomHeightBranch(pressureHub, graph, side) ? { hub: pressureHub, branchSide: side } : null;
    }
    const directBranchSide = getDeckPressureBranchSideForNode(pressureHub, graph, node);
    if (directBranchSide !== "top" && directBranchSide !== "bottom") return null;
    if (!canResizeDeckPressureTopBottomHeightBranch(pressureHub, graph, directBranchSide)) return null;
    if (isDeckPressureTopBottomHeightResizeEdge(node, graph, side)) return { hub: pressureHub, branchSide: directBranchSide };
    const nodeRect = getNodeGraphRect(node);
    if (!nodeRect) return null;
    const edgeY = side === "top" ? nodeRect.y : nodeRect.bottom;
    for (const branchSide of ["top", "bottom"]) {
        if (getDeckPressureBranchAxis(pressureHub, graph, branchSide) !== "horizontal") continue;
        const branchMembers = getDeckPressureBranchMembers(pressureHub, graph, branchSide);
        if (!branchMembers.length) continue;
        const hubY = Number(pressureHub.pos?.[1]) || 0;
        const hubH = Number(pressureHub.size?.[1] ?? pressureHub.properties?.nodeSize?.[1]) || 0;
        const targetY = branchSide === "top" ? hubY : hubY + hubH;
        if (Math.abs(edgeY - targetY) <= 4) return { hub: pressureHub, branchSide };
    }
    return null;
}

function getDeckPressureHubTopBottomSeamResizeTarget(node, graph, anchor) {
    if (!isDeckPressureHub(node) || (anchor !== "top" && anchor !== "bottom")) return null;
    const branchSide = anchor;
    if (getDeckPressureBranchAxis(node, graph, branchSide) !== "horizontal") return null;
    const target = getDeckPressureBranchMembers(node, graph, branchSide)[0] || null;
    if (!target) return null;
    return {
        node: target,
        anchor: branchSide === "top" ? "bottom" : "top",
    };
}

function getDeckPressureTopBottomSeamResizeTarget(node, graph, anchor) {
    const seam = resolveDeckPressureTopBottomSeam(node, graph, anchor);
    if (!seam) return null;
    if (isDeckPressureHub(node)) return getDeckPressureHubTopBottomSeamResizeTarget(node, graph, anchor);
    // All members of a horizontal row share y and height, so any member can
    // act as the resize entity for the branch — use the first for a stable
    // session target while the pointer travels across members.
    const branchMembers = getDeckPressureBranchMembers(seam.hub, graph, seam.branchSide);
    if (getDeckPressureBranchAxis(seam.hub, graph, seam.branchSide) !== "horizontal") return null;
    const targetAnchor = seam.branchSide === "top" ? "bottom" : "top";
    const target = branchMembers[0] || null;
    return target ? { node: target, anchor: targetAnchor } : null;
}

function isDeckPressureHubSeamTopBottomEdge(node, graph, side) {
    return !!resolveDeckPressureTopBottomSeam(node, graph, side);
}
...
export function createDerpShield(node) {
    ...
    const resizeHandleSharedTop = document.createElement("div");
    resizeHandleSharedTop.style.cssText = `
        position: absolute;
        left: 0;
        top: 0;
        width: 100%;
        ...
        pointer-events: none;
    `;
    shield.appendChild(resizeHandleSharedTop);

    const resizeHandleSharedBottom = document.createElement("div");
    resizeHandleSharedBottom.style.cssText = `
        position: absolute;
        left: 0;
        bottom: 0;
        width: 100%;
        ...
        pointer-events: none;
    `;
    shield.appendChild(resizeHandleSharedBottom);
    ...
    shield._resizeHandleSharedTop = resizeHandleSharedTop;
    shield._resizeHandleSharedBottom = resizeHandleSharedBottom;
    ...
    const clearDockResizeActiveMembers = (resizeNode) => {
        ...
        const clearPressureSession = (member) => {
            ...
            if (pressureHub) {
                ...
                pressureHub._deckPressureTopBottomResizeSession = null;
        ...
        if (activeMembers instanceof Set) {
            activeMembers.forEach((member) => {
                ...
                member._horizontalDeckWidthResizeValue = null;
    ...
    const startResize = (e, anchor = "bottom-right") => {
        ...
        const pressureFrameEdge = !isCornerResizeAnchor(anchor) && isDeckPressureFrameEdge(node, graph, anchor);
        if (isDeckPressureBranchMember(node, graph) && isCornerResizeAnchor(anchor) && !pressureFrameCorner) {
            shield.onpointerdown(e);
            return;
        }
        // Branch member side-edge hits only resize when on the deck frame
        // boundary or an internal seam; otherwise pass through so clicks
        // reach the node body. The shield's _frameEdge* flags (set during
        // syncDerpShield) tell us whether a visible frame-edge handle exists
        // for this side — if so, the click came from that handle and must
        // proceed. For hub-facing seams, getDeckPressureSideSeamResizeTarget
        // detects the internal seam. Top/bottom anchors on side-branch
        // members are either deck frame edges (caught by _frameEdge flags)
        // or internal vertical seams between branch siblings, both of which
        // must proceed. Side anchors on top/bottom (or side-horizontal)
        // branch members are internal horizontal seams between siblings —
        // canResizeHorizontalSharedEdge is the same predicate the hover and
        // hit-test paths use, so drag start stays in sync with the ghost and
        // cursor. Passing through on a real seam would re-enter
        // shield.onpointerdown, re-detect the same seam, and recurse.
        const isSideAnchor = anchor === "left" || anchor === "right";
        const isTopBottomAnchor = anchor === "top" || anchor === "bottom";
        const hasVisibleFrameEdge = isSideAnchor
            ? !!(shield._frameEdgeLeft && anchor === "left") || !!(shield._frameEdgeRight && anchor === "right")
            : !!(shield._frameEdgeTop && anchor === "top") || !!(shield._frameEdgeBottom && anchor === "bottom");
        const hubSeamResizeEarly = isSideAnchor
            ? getDeckPressureSideSeamResizeTarget(node, graph, anchor)
            : (isTopBottomAnchor ? getDeckPressureTopBottomSeamResizeTarget(node, graph, anchor) : null);
        const internalSeamResize = isSideAnchor ? canResizeHorizontalSharedEdge(node, graph, anchor) : false;
        if (isDeckPressureBranchMember(node, graph) && isSideAnchor && !isCornerResizeAnchor(anchor) && !pressureFrameEdge && !hasVisibleFrameEdge && !hubSeamResizeEarly && !internalSeamResize) {
            shield.onpointerdown(e);
            return;
        }
        ...
        isResizing = true;
        const pressureHub = graph ? getDeckPressureHubForNode(node, graph) : null;
        // Frame edge resize does NOT route to the hub — the branch member
        // itself is the resize target. When a left/right edge of a branch
        // member aligns with the frame boundary, applyDeckPressureSideWidthResize
        // in syncDockResizePair handles it: it resizes the entire branch width
        // (all stack members) and adjusts the hub position/size accordingly.
        // Corner resize still routes to the hub for diagonal frame resize.
        const routeToPressureHub = pressureHub && pressureHub.id !== node.id && pressureFrameCorner;
        const hubSeamResize = hubSeamResizeEarly || (isSideAnchor
            ? getDeckPressureSideSeamResizeTarget(node, graph, anchor)
            : (isTopBottomAnchor ? getDeckPressureTopBottomSeamResizeTarget(node, graph, anchor) : null));
    ...
    resizeHandleSharedTop.onpointerdown = (e) => startResize(e, "top");
    resizeHandleSharedBottom.onpointerdown = (e) => startResize(e, "bottom");
    ...
    const getSharedResizeAnchorAtPointer = (e) => {
        ...
        if (localY <= vEdgeWidth && resolveDeckPressureTopBottomSeam(node, graph, "top")) return "top";
        if (localY >= rect.height - vEdgeWidth && resolveDeckPressureTopBottomSeam(node, graph, "bottom")) return "bottom";
    ...
    const updateSharedResizeHoverSession = (e) => {
        ...
        const neighbor = anchor && graph
            ...
            : null;
        const seam = anchor ? (resolveDeckPressureSideSeam(node, graph, anchor) || resolveDeckPressureTopBottomSeam(node, graph, anchor)) : null;
        const next = seam
            ? { side: anchor, entityId: node.id, neighborId: seam.hub.id, hubId: seam.hub.id, branchSide: seam.branchSide, deckPressureSideWidth: seam.branchSide === "left" || seam.branchSide === "right", deckPressureTopBottomHeight: seam.branchSide === "top" || seam.branchSide === "bottom" }
...
function getShieldResizeCursor(node, graph, anchor) {
    // Pressure hub frame edge resize: always return the axis cursor because
    // the shield sync code only shows the edge handle when the resize is
    // eligible (stack resize or standalone deck).
    if (isDeckPressureHub(node) && (anchor === "left" || anchor === "right")) return "ew-resize";
    if (isDeckPressureHub(node) && (anchor === "top" || anchor === "bottom")) return "ns-resize";
    ...
    const canUseW = side
        ...
        : canW;
    // Vertical stack boundary corners can resize the STACK height even when THIS
    // node is preferred-auto-height (delta distributes among manual members), so
    // use canResizeVerticalStackHeight for top/bottom anchors. Horizontal stacks
    // share one height, so canResizeHorizontalStackHeight covers both.
    const verticalSide = anchor === "top" || anchor === "top-left" || anchor === "top-right" ? "top"
        : (anchor === "bottom" || anchor === "bottom-left" || anchor === "bottom-right" ? "bottom" : null);
    const canUseH = (isHorizontalDockStack
        ? canResizeHorizontalStackHeight(node, graph)
        : (isVerticalDockStack && verticalSide ? canResizeVerticalStackHeight(node, graph, verticalSide) : canH))
        || isDeckPressureHubSeamTopBottomEdge(node, graph, verticalSide);
...
function disableResizeHandles(shield) {
    [
        ...
        shield?._resizeHandleSharedTop,
        shield?._resizeHandleSharedBottom,
    ].forEach((handle) => {
        if (!handle) return;
...
export function syncDerpShield(node) {
    if (!node.interactionShield) return;
    if (!node.interactionShield._resizeHandleSharedLeft || !node.interactionShield._resizeHandleSharedRight || !node.interactionShield._resizeHandleSharedTop || !node.interactionShield._resizeHandleSharedBottom) {
    ...
    const hubNeighborTopForHash = pressureHubForHash ? (getNodeOnDeckEdge(pressureHubForHash, graphForHash, "top")?.id ?? "n") : "n";
...
// --- 1 & 2. DEBUG LAYER & HITBOX RENDER MODES ---
    ...
    if (node.isSystemPanel || node.isSysPanel) {
        ...
        // among manual members) even when THIS node is preferred-auto-height, so
        // use canResizeVerticalStackHeight for top/bottom boundary corners instead
        // of the per-node canH. Horizontal stacks share one height across all
        ...
        if (node.interactionShield) {
            node.interactionShield._frameEdgeLeft = deckFrameEdgeLeft;
            node.interactionShield._frameEdgeRight = deckFrameEdgeRight;
            node.interactionShield._frameEdgeTop = deckFrameEdgeTop;
            node.interactionShield._frameEdgeBottom = deckFrameEdgeBottom;
        }
        if (node.interactionShield._resizeHandleSharedLeft) {
            ...
            sharedLeftStyle.width = `${SHARED_HORIZONTAL_SEAM_HITBOX_PX}px`;
        ...
        if (node.interactionShield._resizeHandleSharedRight) {
            ...
            sharedRightStyle.width = `${SHARED_HORIZONTAL_SEAM_HITBOX_PX}px`;
        ...
        handleStyle.cursor = (isPressureMember && allowFrameCornerBottomRight) ? "nwse-resize" : getCornerCursor(canUseRightW, "nwse-resize", canUseBottomCornerH);
        // THE INTERACTION GUARD: Disable handle interaction entirely if both axes are auto-managed
        node.resizable = canResizeStackW || canResizeStackH || canResizeSharedW || hasFrameEdge || canResizePressureSeamTopH || canResizePressureSeamBottomH || !(vars.autoWidth && vars.autoHeight); // THE NATIVE FIX: Kill LiteGraph's own resize logic
        const showBottomRightCorner = node.resizable && (canUseRightW || canUseBottomCornerH) && allowBottomResizeCorners && allowFrameCornerBottomRight;
        ...
        if (allowBottomResizeCorners && ((canResizeSharedRightW && !isVerticalDockStack && !isPressureHub) || canResizePressureSeamRightW || canResizePressureSideHorizontalRightW) && canUseRightW) {
            handleStyle.width = `${SHARED_HORIZONTAL_SEAM_HITBOX_PX}px`; handleStyle.height = `${isPressureMember ? pressureSharedEdgeHeight : visualH * scale}px`; handleStyle.cursor = "ew-resize"; handleStyle.display = "block"; handleStyle.pointerEvents = "auto";
        ...
        if (node.interactionShield._resizeHandleLeft) {
            ...
            leftStyle.cursor = (isPressureMember && allowFrameCornerBottomLeft) ? "nesw-resize" : getCornerCursor(canUseLeftW, "nesw-resize", canUseBottomCornerH);
            const showBottomLeftCorner = node.resizable && (canUseLeftW || canUseBottomCornerH) && allowBottomResizeCorners && allowFrameCornerBottomLeft;
            ...
            if (allowBottomResizeCorners && ((canResizeSharedLeftW && !isVerticalDockStack && !isPressureHub) || canResizePressureSeamLeftW || canResizePressureSideHorizontalLeftW) && canUseLeftW) {
                leftStyle.width = `${SHARED_HORIZONTAL_SEAM_HITBOX_PX}px`; leftStyle.height = `${isPressureMember ? pressureSharedEdgeHeight : visualH * scale}px`; leftStyle.cursor = "ew-resize"; leftStyle.display = "block"; leftStyle.pointerEvents = "auto";
        ...
        if (node.interactionShield._resizeHandleTopLeft) {
            ...
            const showTopLeftCorner = showTopCorners && (canUseLeftW || canUseTopCornerH) && allowTopResizeCorners && allowFrameCornerTopLeft && !hasSharedLeftEdge;
            ...
            if (isVerticalDockStack && hasInternalTopResizeEdge && canH) {
                ...
                topLeftStyle.cursor = (isPressureMember && allowFrameCornerTopLeft) ? "nwse-resize" : getCornerCursor(canUseLeftW, "nwse-resize", canUseTopCornerH);
            }
        }
        ...
        if (node.interactionShield._resizeHandleTopRight) {
            ...
            const showTopRightCorner = showTopCorners && (canUseRightW || canUseTopCornerH) && allowTopResizeCorners && allowFrameCornerTopRight && !hasSharedRightEdge;
            ...
            if (isVerticalDockStack && hasInternalTopResizeEdge && canH) {
                ...
                topRightStyle.cursor = (isPressureMember && allowFrameCornerTopRight) ? "nesw-resize" : getCornerCursor(canUseRightW, "nesw-resize", canUseTopCornerH);
            }
        }
    }
