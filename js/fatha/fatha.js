...
const LORA_STACK_DEBUG_FRAME_MS = 12;
const LORA_STACK_DEBUG_THROTTLE_MS = 1000;
...
function logLoraStackFramePerf(node, perf) {
    ...
    const deckText = `${node.properties?.deckParentId || 'none'}:${node.properties?.deckDockSide || 'none'}`;
    console.log(`[LoraStackPerf] frame node=${node.id}:${node.titleLabel || node.title || node.type} zoom=${(app.canvas?.ds?.scale || 1).toFixed(2)} total=${perf.total.toFixed(1)} cache=${cache} path=${perf.cachePath || 'draw'} draw=${perf.cacheDraw || 'none'} deck=${deckText} presH=${perf.preserveHorizontalDeckHeight ? 1 : 0} lockH=${perf.lockHorizontalDeckResize ? 1 : 0} sync=${perf.syncReason || 'none'} shouldSync=${node._shouldSync ? 1 : 0} force=${node._forceSync ? 1 : 0} dirty=${node._layoutDirty ? 1 : 0} size=${Math.round(node.size?.[0] || 0)}x${Math.round(node.size?.[1] || 0)} stack=${stackCount} phases=${phaseText} slow=${slowWidgets}`);
}
...
function drawPassiveWholeWallCache(node, ctx, cacheCanvas, cacheReg, fallbackScale = 1, debugInfo = null) {
    if (!cacheCanvas || !cacheReg) return false;
    const setDebugDraw = (text) => {
        if (debugInfo) debugInfo.cacheDraw = text;
    };
    ...
    if (!visible) {
        setDebugDraw(`full:${regW}x${regH}@${Math.max(1, Number(fallbackScale) || 1).toFixed(2)}`);
    ...
    setDebugDraw(`src:${sourceW}x${sourceH}@${cacheScale.toFixed(2)} dst:${Math.round(sourceW / cacheScale)}x${Math.round(sourceH / cacheScale)}`);
...
export function fatha(nodeType, nodeData, minWidth = 100) {
    ...
    nodeType.prototype.onDrawForeground = function(ctx) {
        ...
        if (loraStackPerf) {
            loraStackPerf.preserveHorizontalDeckHeight = !!preserveHorizontalDeckHeight;
            loraStackPerf.lockHorizontalDeckResize = !!lockHorizontalDeckResize;
            loraStackPerf.syncReason = [hasLayoutChanged ? 'layout' : '', hasVisualChanged ? 'visual' : '', this._forceSync ? 'force' : '', this._layoutDirty ? 'dirty' : ''].filter(Boolean).join('+') || 'none';
        }
        ...
        if (loraWidgetLayer.canUse && loraWidgetLayerCache?.key === loraWidgetLayer.key && loraWidgetLayerCache?.canvas) {
            if (loraStackPerf) loraStackPerf.cachePath = "widget-layer";
            drawPassiveWholeWallCache(this, ctx, loraWidgetLayerCache.canvas, loraWidgetLayer.cacheReg, loraWidgetLayerCache.scale || passiveCacheScale, loraStackPerf);
        ...
        if (canUsePassiveWholeWallCache && passiveWholeWallCache?.key === passiveWholeWall.key && passiveWholeWallCache?.canvas) {
            if (loraStackPerf) loraStackPerf.cachePath = "whole-wall";
            drawPassiveWholeWallCache(this, ctx, passiveWholeWallCache.canvas, passiveWholeWall.cacheReg, passiveWholeWallCache.scale || passiveCacheScale, loraStackPerf);
        ...
        if (this.layout?.regions) {
            ...
            if (cacheCtx) {
                cacheCtx.restore();
                if (loraStackPerf) loraStackPerf.cachePath = "whole-wall-store";
                this[passiveWholeWall.cacheSlot] = {
                    key: passiveWholeWall.key,
                    canvas: cacheCanvas,
                    scale: passiveCacheScale,
                };
                drawPassiveWholeWallCache(this, ctx, cacheCanvas, passiveWholeWall.cacheReg, passiveCacheScale, loraStackPerf);
            }
            if (widgetLayerCtx) {
                widgetLayerCtx.restore();
                if (loraStackPerf) loraStackPerf.cachePath = "widget-layer-store";
                this._loraStackWidgetLayerCanvasCache = {
                    key: loraWidgetLayer.key,
                    canvas: widgetLayerCanvas,
                    scale: passiveCacheScale,
                };
                drawPassiveWholeWallCache(this, ctx, widgetLayerCanvas, loraWidgetLayer.cacheReg, passiveCacheScale, loraStackPerf);
            }
